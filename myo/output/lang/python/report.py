...
def format_location(file: OutputLine[FileLine], location: Location, path_formatter: PathFormatter) -> Do:
    ...
    return List(f'{path}  {location.line}{fun}')


class python_report_event(Case[PythonEvent, NS[CommandRibosome, List[str]]], alg=PythonEvent):
    ...
    def stack_frame(self, event: StackFrameEvent) -> Do:
        ...
        code_line = event.code.map(lambda a: a.text).to_list
        return loc_line + code_line

    def error(self, event: ErrorEvent) -> NS[CommandRibosome, List[str]]:
        return NS.pure(List(event.line.text))
